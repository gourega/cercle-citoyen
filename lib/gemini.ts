      ...
      contents: [
        {
          parts: [
            ...
            { text: `Analyse cette pollution en Côte d'Ivoire. Format JSON: city, sector, nature, status, description, actionPlan, insight.` }
          ]
        }
      ],
    ...
    const response = await ai.models.generateContent({
      ...
      contents: {
        parts: [
          ...
          { text: "Transforme ce site pollué en espace urbain propre et verdoyant." }
        ]
      }
    });
...
export async function summarizeCircleDiscussions(circleType: string, posts: string[]) {
  try {
    ...
    const response = await ai.models.generateContent({
      model: "gemini-3-flash-preview",
      contents: `Synthétise : ${posts.join('\n')}`,
    });
...
export async function analyzeIdeaImpact(title: string, description: string) {
  try {
    ...
    const response = await ai.models.generateContent({
      model: "gemini-3-flash-preview",
      contents: `Analyse l'impact de l'idée : ${title}`,
      config: { responseMimeType: "application/json" }
    });
    return JSON.parse(response.text || "{}");
...
export async function analyzeCommunityReputation(entityName: string, vouches: string[]) {
  try {
    ...
    const response = await ai.models.generateContent({
      model: "gemini-3-flash-preview",
      contents: `Analyse réputation de ${entityName}`,
      config: { responseMimeType: "application/json" }
    });
    return JSON.parse(response.text || "{}");
...
export async function mediateChat(messages: {sender: string, text: string}[]) {
  try {
    ...
    const response = await ai.models.generateContent({
      model: "gemini-3-flash-preview",
      contents: `Agis en médiateur : ${messages.map(m => m.text).join('\n')}`,
    });
    return response.text;
...
export async function getConsensusSummary(messages: {sender: string, text: string}[]) {
  try {
    ...
    const response = await ai.models.generateContent({
      model: "gemini-3-flash-preview",
      contents: `Quel consensus ? ${messages.map(m => m.text).join('\n')}`,
    });
    return response.text;
...
export async function generateImpactVisual(prompt: string) {
  ...
  const response = await ai.models.generateContent({
    model: 'gemini-2.5-flash-image',
    contents: { parts: [{ text: `Visual for Ivory Coast project: ${prompt}` }] },
