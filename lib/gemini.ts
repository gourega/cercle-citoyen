...
const getAI = () => {
  const key = (window as any).process?.env?.API_KEY || (import.meta as any).env?.API_KEY;
  if (!key) {
    console.error("API_KEY manquante dans l'environnement");
...
export async function generateMobilizationVideo(prompt: string) {
  ...
  let operation = await ai.models.generateVideos({
    ...
    prompt: `Inspiring mobilization video: ${prompt}`,
